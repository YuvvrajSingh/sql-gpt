...
class EnhancedSQLAssistant:
    ...
    def process_query_with_agent(self, natural_language_query):
        """Process query using SQL Agent for intelligent handling"""
        if not self.agent:
            return None, None, "SQL Agent not initialized. Please connect to database and AI first."
        
        try:
            # Create a container for the streamlit callback
            with st.container():
                streamlit_callback = StreamlitCallbackHandler(st.container())
                
                # Use the agent to process the query
                response = self.agent.run(natural_language_query, callbacks=[streamlit_callback])
